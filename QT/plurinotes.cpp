...
void NotesManager::addArticle(const QString& id, const QString& ti, const QString& te){
    for(unsigned int i=0; i<nbArticles; i++){
        if (articles[i]->getId()==id) throw NotesException("Erreur : identificateur d�j� existant");
    }
...
void NotesManager::save() const {
    ...
    for(unsigned int i=0; i<nbArticles; i++){
        ...
        //stream.writeTextElement("date_creation",articles[i]->getDateCreation());
       // stream.writeTextElement("date_modif",articles[i]->getDateLastModif());
